...
As explained in the introduction, detecting more efficiently individual variation in plasticity (I * E) in natural condition is essential to better understand individual responses to climate change. However, although some good methods exists today [@nusseyEvolutionaryEcologyIndividual2007; @ramakersProbingVariationReaction2023], a lot of biases coming from environmental proxies still limit these methods today.

As said in the introduction again, we believe that using Double Hierarchical Generalized Models (DHGLM) can drastically reduce these problems and help us improve the efficiency of I * E detection in natural populations.
...
Expected results @fig-ie-vs-no-ie, unless very specific case (i.e., exact opposite effect, @fig-ie-vs-opposed)

But watch out for unbalanced model which can give the same results even though there is no I * E (@fig-balanced-unbalanced)


<!-- Simulations to see how we can estimate it (data with and without I * E => fit the models, check how the different parameters reacts and how we interpret it, and also how we can present them). Simulation to test the different bias (Sampling bias, etc.)
