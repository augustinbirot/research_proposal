...
- using JM as key work for TODO tree extension. it has to been at the beginning of a line or at the start of a comment
- comments using \<\!\-\- JM blablabla \-\-\> <!-- JM blablabla -->
:::

<!--JM au final il ya des petites coqui;les en anglais et des choses que je raffinerai pour un proposé final mais il faut vraiment l'envoyer aujourd'hui ou demain matin pour ton comité. Je pnse que c'est bon comme c'est et que tu auras des questions à clarifier pendant ton TAC mais globalement c'est très bon. -->
...
We know that body mass is strongly related to both temperature acclimatation [i.e., thermoregulation, @riesenfeldRoleBodyMass1981; @kurzPhysiologyThermoregulation2008] and food abundance [@acquaroneEffectsFoodAbundance2002]. Therefore, we need to study body mass in that context of climate and ecological changes.
To study such changes, we rely on Life-History Theory. This framework is used to study how ressources are allocated to specific traits to cope with environment while maximizing individual fitness. The theory rely on the fact that ressources are limited and trade-off must be maid between different traits to achieve the best life-history strategy [@Roff1992TheEO; @10.1093/oso/9780198577416.001.0001; @bellCostsReproductionTheir1980]
...
Therefore, we expect changes in body mass and, we need to understand what is driving it and how it is is happening.
...
When fitting a DHGLM on a focal phenotypic trait with multiple observations for each individuals, an absence of I * E (i.e., each individual's phenotypic response will be the same, @fig-ie-vs-no-ie a) should show and absence of among-individual variance in the residual variance ($V_{V_e} = 0$) as each individual will exhibit the same range of phenotypic values ([Fig. @fig-ie-vs-no-ie] c). <!--JM missing end of sentence ??-->
...
5. From Mass to Manner: How Body Mass and Age Shape Behavior in Yellow-Bellied Marmots. [A Birot, D Blumstein and JGA Martin]{.underline}. Targeted journal: *Proc. R. Soc. B.*; *Oikos*
