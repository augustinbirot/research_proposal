...
As such, changes in LHTs are expected to have even greater impacts on the global phenotype. A robust method to study such changes are **Multivariate Animal models** (i.e., an Animal model with multiple phenotypic traits as dependant variables) which allows us to estimate the genetic covariance between each trait [@teplitskyEvolutionaryPotentialConstraints2014].
