...
<!--JM: avoid starting a paragraph with however. The word is used to link to ideas adn thus the 2 ideas should be part of the same paragraph. Try to keep in ming 1sentence = 1 idea and 1 paragraph = 1 topic. So no need to create anew paragraph here -->
<!--JM: maybe you can point out how following that definition that any traits under selection is a life-history traits which would be really different from the first idea of life-history traits from Roff and Stearns (poorly defined concept) -->
...
<!--JM: yark (minor comment more for style) 2 long sentences only for an entire paragraph. Please split that into multiple sentences an unpack the ideas-->

With this <!--JM: which one? unclear --> definition of life-history traits, both all and no traits could be considered as such. Furthermore another objection with this definition is that with it, life-history traits both define and are defined by fitness, therefore making this reasoning tautological.

Reading the literature, I realize that a lot of authors working on life history traits don't explicitly define them, thus the community would benefit from a consensus on a clearer definition of Life History Trait if possible. However, Life-History traits usually recognized as being individuals life-cycle features such as size at birth; age at maturity; size at maturity; number and size of offspring [@Roff1992TheEO; @stearns92].<!--JM: not sure I understand this sentence. also you need a working definition of life-history trait for your thesis so just use one and stick with it-->

<!--JM: no need to use uppercases for Life History Trait. Please edit throughout-->
...
Body mass plays a critical role for most species. It affects individual metabolic rate [@darveauAllometricCascadeUnifying2002] as well as performance (e.g., fighting, competition ability <!--JM: REF??-->). For instance, @weibelAllometricScalingMaximal2004 showed a strong correlation between maximal metabolic rate and body mass in mammals.

Body mass is also strongly related to an individual energetic reserves, thereby impacting resilience to environmental challenges by buffering against seasonal food scarcity [@heldstabHowMammalsBuffer2017]. More generaly, body fat can be considered a buffer against harsh environments [@denryterSurvivalFattestHow2022].
<!--JM: also think that being too big can create thermoregulation issue when too hot-->
Extending this reasoning to a wider timescale, we can expect that larger individuals may better buffer resource-poor years, increasing their resilience to environmental variability and unpredictability [@einum2004environmental].
...
* [mutation]{.underline}, which introduces new genetic variation through changes in the DNA sequence<!--REF-->;
* [migration]{.underline}, which maintains gene flow between subpopulations, both inssuring genetic homogenization and bringing new variation in the population<!--REF-->;
...
Changes in such <!-- JM: such ... such. also which traits are you refering to --> traits are expected to go along with other changes, therefore synonym of greater impacts on the global phenotype<!-- JM: extremly vague. I am not sure I understand what you mean here -->. A robust method to study such<!-- JM again such, highly confusing. Suggestion: Arobust method to study coevolution of multiple traits is ...--> changes are Multivariate Animal models which allows us to estimate the genetic covariance between each trait by simultaneously model multiple traits as dependant variables[@kruukEstimatingGeneticParameters2004].
...
Marmots experiences high seasonal fluctuation in body mass, with a critical threshold to be reached before the onset of hibernation in order to first, survive through the next active season and second, have enough energy left for reproduction [which occurs in the first weeks of the active season, @armitageVernalBehaviourYellowbellied1965; @armitageMarmotBiologySociality2014]. Hence, body mass is a crucial traits for the marmots.
...
Body mass is a highly plastic trait, particularly for marmots, as it experiences considerable fluctuations throughout the active season. It is, for obvious reasons, impossible to record all individual body mass at the same time. Therefore, it is necessary to estimate it for each individual at the same time of the year. Using repeated measures for each individual throughout each active season, a linear mixed model was fitted and its Best Linear Unbiased Predictors were used to extract each individual’s body mass at the beginning and end of each years' active seasons, so June 1^st^ and August 15^th^ [details in @ozgulCoupledDynamicsBody2010; @jebbBiggerNotAlways2021 <!-- JM also cite Martin Pelletier 2011 -->]. Over 61 years of observations (1962 - 2022), we have 7,586 body mass estimations for 4,656 individuals.
...
Then, I want to understand the causes of this <!-- JM which one the stuff you are referring to is quite far away know and most reader would be confused here -->change. After having study the body mass at the beginning and end of the active season, I will be able to test the effect of active, hibernation seasons and season length on body mass increase (during active season) and loss (during hibernation).
...
Detecting individual variation in plasticity is challenging due to the unknown aspect of the environmental variables organism are responding to [@nusseyEvolutionaryEcologyIndividual2007; @ramakersProbingVariationReaction2023]. Although some methods exist today, a lot of biases coming from environmental proxies still limit these methods <!--JM avoid the repeat today -->. DHGLMs are a promising avenue to help the study of *I \* E* in natural populations, but an investigation to reveal its potential and limits is needed.
...
Although detecting that $V_{V_e}$ is different from 0 isn't a proof of *I \* E* in itself, as the variation could be due to other processes, it is a necessary condition of *I \* E* <!-- (except under very unlikely conditions @fig-opposed_unbalanced a & b) JM honestly it works only if you have 2 individuals or only 2 potential slopes so i don<t think it is even necessary to mention-->. Finding $V_{V_e} > 0$ would thus suggest that investigating I * E and looking for the unknown E is a worthwhile investigation. 

Finally, it is worth noting a potential bias with this method that must be taken into account when performing such analysis. When individuals are sampled on different range of environmental values then the within-indivdiual variance in residual variance could be different from zero despite an absence of *I \* E* (@fig-opposed_unbalanced c, d). However, adding the environment as a fixed effect in the model would remove this bias.
...
<!-- JM at some point I think it would be good to have a diagram of the marmot yearly cycle and what can impact body mass and why we can see and expect temporal lags forcing us to havea look at the summer mass gain and the winter loss. also be ready to discuss physiological limitations of mass gain and mass loss -->
...
The first step is going to be to gather environmental data. I will be able to extract and estimate hibernation and active season length, seasonal conditions (average snowpacks, average temperatures and precipitation), food availability, and drought frequency and severity over the last 50 years.<!-- Jm comps note: be ready to explain what and how you get those data. remote sensing, explain them and what are the potential limitations of those type of data -->
...
<!--Using our environmental records at RMBL, I will define each year of study as either "good", "mid" or "poor" in terms of resources and environmental conditions. Given that the mean number of offsprings per year, per female in our population are of, let's say, [3] in the up valley and [5] in down valley-->
<!-- AB - Dummy number, do you have the real ones???? JM: you can get them from the database-->,
<!-- I will simulate two populations with the same means but different variance.

 The first population will show large variance in litter size associated to variation in the environment. In a "poor" year, each female will produce 1 offspring in the up valley and 3 in down; in "good" years: 5 and 7; and "mid" years: 3 and 5.
In the second population, reproduction will be less afftec by the en during "poor" years each female will produce 2 offsprings in the up valley and 4 in the down valley; in "good" years it will be 4 and 6; and "mid" years: 3 and 5. 

JM: I started to work on that section to realize that I don<t think those numbers are appropriate and anyway you don<t need those details now-->
...
<!-- JM not sure to see what you mean here -->
