...
However, this is a wide definition that could correspond to almost any traits as most phenotypic trait could impact either survival [e.g., <!-- TODO: find traits, other than body mass, that impact individual survival -->; bold and active trinidanian guppies, *Poecilia reticulata*, survive longer against predators; @smithBehavioralTypesPredictors2010] or reproductive sucess [e.g., <!-- TODO: find traits, other than body mass, that impact individual survival -->; or courtship ability <!-- bird of paradise --> **REF**]. But conversely, one could argue that no trait would impact directly, *per se*, an individual fitness (e.g., does body mass in itself directly impact over-hibernation survival in yellow-bellied marmots, or is it percentage of fat-tissue, or metabolic capacity, or individual thermoregulation capacity?).

Hence this definition encompass both all and no traits. Furthermore another objection with this definition is that with it, life-history trats both define and are defined by fitness, therefore making this reasoning tautological.
...
We know that body mass is strongly related to both temperature acclimation [i.e., thermoregulation, @riesenfeldRoleBodyMass1981; @kurzPhysiologyThermoregulation2008] and food abundance [@acquaroneEffectsFoodAbundance2002]. Therefore, we need to study body mass in that context of climate and ecological changes.

<!-- STRUCTURE: What is the role of body mass? What does it impact? -->
Body mass plays a crucial role in many species. Individuals with greater body mass will have more energetic reserves, making them more resilient to environmental pressures, as they will be able to buffer against seasonal food scarcity [@heldstabHowMammalsBuffer2017]. More generally, body fat can be considered as a buffer against harsh environment [@denryterSurvivalFattestHow2022]. Extending this reasoning to a wider timescale, we could expect these bigger individuals to be able to buffer poor years in terms of resources, and so increasing their resilience to environmental variability [@einum2004environmental].


- body mass explain 89% of the hibernation survival in juvenile yellow-bellied marmots, *Marmota flaviventer* [@armitageMarmotBiologySociality2014]

- <!-- TODO: add another example of impact on survival -->

- impact of winter body mass variation on female reproductive success in Norwegian moose, *Alces alces* [@milnerReproductiveSuccessFailure2013]

- large body size and mass is correlated to higher mating success in northern elephant seals, *Mirounga angustirostris* [@crockerImpactBodyReserves2012]

- ndividuals with greater energy reserves have a better capacity to reproduce, as illustrated in bighorn sheeps, *Ovis canadensis* [@festa-bianchetMassDensityDependentReproductive1998]


<!-- STRUCTURE: WHY does it have this role and impact those things -->
<!-- TODO: Add a § about body mass ecology and physiology & link that with the role of body mass -->
<!-- TODO: Body captures height (important for fitness in some species for access to reproduction) but also energetic reserve (impact on survival) => Mass is therefore important to study -->

However, a too large body mass can become a handicap. Even though some handicaps have been theorized to be an asset in sexual selection [@zahavi_handicap], beyond a certain threshold, too large individuals will be counter-selected as it could induce a cost in individual performance [e.g., capacity to run from predators, @jebbBiggerNotAlways2021].

Overall, body mass can be considered a crucial trait influencing directly both survival and reproduction in many species, but a balance must be reach between performances and costs, and with investment in other traits.

"Although some could argue that body mass can be considered as a Life History Traits in some species as it can impact directly the fitness of an individual, due to the lack of a clear definition, I will just say that Body mass can be seen as a NODE impacting other traits, finally impacting global fitness"
...
<!-- TODO: potential switch between hibernant § and effect of global warming § -->
<!-- TODO: effect on survival over hibernation, and survival/reproduction at emergence -->
<!-- TODO: effect of global warming on hibernant =>  body mass could be less "important" because less "pressure" -->
...
<!-- TODO: observed changes not necessarely adaptive as environmental constraints with hibernants are changing, so changes are not necessarly adaptive -->
...
However, such models are heavily data-hungry, and the main reason that significant results with such methods are quite rare today is that only a few studies have enough data to support the statistical power required for these complex models [@lynchGeneticsAnalysisQuantitative1998] <!-- TODO: Lynsh & Walsh (data hungry, which constrait the power associated to a lot of studies) -->. This is one of the main reason long-term study datasets in the wild is essentiel <!-- TODO: rephrase --> brings a lot of expectation to address such limitations [@teplitskyEvolutionaryPotentialConstraints2014].
<!-- TODO: Look at preface and first chapter of QG in the wild -->
...
<!-- TODO: 3 models (boldness genetic value; boldness*body mass genet correlation; effect of body mass * age on boldness (this one isn't an animal model)) -->
...
- I will first estimate the heritability and genetic variation of boldness over cohorts using an univariate animal model (@eq-mod-ch4-m1)
...
- Then I will estimate the genetic correlation between boldness and body mass over cohorts using a bivariate animal model (@eq-mod-ch4-m2)
...
<!-- TODO: Pressure problem: Stearns => gradiant = pression; Lande & Arnold 1983 (the measurement of selection on correlated characters): gradient = forces of selection on the trait ; selection gradient capture selection forces (Lande et arnold); Walsh & lynsh 2022 use selection pressure in general ; so I am in line with the litterature
Pression : force divided by the area of the surface on which its applied 
TODO: add the definition in text (Stearns) & use lande & arnold in oral presentation -->
