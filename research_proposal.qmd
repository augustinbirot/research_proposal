...
Phenotypic plasticity refers to the capacity of a single genotype to express differnt phenotypes depending on environmental condition. It allows for rapid, reversible responses within an individual's lifetime, is highly flexible, and does not involve any changes at the genetic level [@pigliucciPhenotypicPlasticityNature2001].

In contrast, evolution is defined as a change in allele frequencies in a population over time. It is driver by four fundamental forces: mutation, which introduces new genetic variation through changes in the DNA sequence<!--REF-->; migration, which maintains gene flow between subpopulations, both inssuring genetic homogenization and bringing new variation in the population<!--REF-->; genetic drift, which is random fluctuations in allele frequencies over generation resulting from finite population size, and can lead to evolutionary change without selection; and natural selection. For this latter to drive evolutionary change, three conditions must be met: there must be phenotypic variation among individuals, this variation must be associated with differences in fitness, and these differences must be transmissible to the next generations (i.e., heritability) [@lewontinUnitsSelection1970]. Whene these conditions are satisfied, individuals with advantageous phenotypes will have a higher reproductive succes, thus transmitting their genes across generation, eventually increasing in frequence in the population [@lynchGeneticsAnalysisQuantitative1998].
...
On the other hand, evolution rely on genetic change, therefore evolution can be studied by looking at variation at a genetic level.
