...
Mean cohort’s body mass predicted breeding values (i.e., genetic value) trend over 5,000 iterations (gray lines). The median trends of the observations (red line) and under a null scenario (i.e., with only genetic drift occurring, black line) are represented, according to linear (solid line) or generalized additive models (dashed line). Points and error bars represent the median and 95% credibility interval of the posterior mean predicted breading values for each cohort.
:::
...
To investigate the body mass increase in the last-half century, the first step is to understand what happened precisely. By understanding the mechanism of this increase, I mean understand if cohorts now have a bigger constitution than before (i.e., are they born bigger), or does their growing capacity has increased through time?

To explain that in statistical term, if we look at the individual body mass over time, I want to know if the intercept (constitution), the slope (growing) aor both have changed over the time cohorts.

Body mass in juvenile was predicted to stay stable as it favour a higher running speed allowing juvenile to escape predators more efficiently during foraging [@jebbBiggerNotAlways2021]. This was one of the main argument to explain why yellow-bellied marmots body mass was (before the 70s) stable in adults, the "invisible fraction" explained by @jebbBiggerNotAlways2021.
...
# Identifying the predictors of increased body mass. {#sec-chap3}
