...
The good news is that a promising, fairly new, statistical method could bring new interesting insights to that matter: the "Double Hierarchical Generalized Linear Model" (DHGLM). DHGLM is a kind of mixed model fitting random effects on both the mean and dispersion. In other words, it models the distribution for both the mean and the residual variance [@leeDoubleHierarchicalGeneralized2006]. So, DHGLM estimates the *variance in the residual variance* (i.e., $\boldsymbol{V_{V_e}}$) corresponding to the variance in the proportion of the residual variance attributable to each individual.

We are confident that DHGLM can be applied to developp a more powerfull method to detect individual variation in plasticity (${I * E}$) in natural systems. Indeed, a $V_{V_e}$ significantly greater than 0 should indicate that there is some discrepencies in the individual contribution to the residual variance. [..] <!-- I need a sentence or two to make the link biologically why it means I*E =>-->As this show a different contribution of each individual to the residual variance in phenotype, it shows that the range of phenotype expressed by each individuals is different, ergo it can show an individual variance in their plastic response, so ${I * E}$.

Although a non-zero variance isn't necessarly due to the presence of ${I * E}$ in the system, it would justify investigation by testing different environmental proxies. In contrast, except in very specific conditons, almost impossible in natural conditions (i.e., **perfect** contrary effect, @fig-ie-vs-opposed), a $V_{V_e}$ equal to 0 indicate that the relative contribution to the residual variance in phenotype isn't significantly different between each individual, and so that there's no individual variation in plasticity in the studied system.

An important feature of this method is that it should be free from the problems of bad proxies as it detects ${I * E}$ from the structure of the residual variance. And obviously, as for the random regreession with the RRAM, we can adapt this method we quantitative genetics to also detect ${G * E}$ if ${I * E}$ is detected by combining a DHGLM with an Animal model, giving a "Double Hierarchical Animal Model" (DHAM).

This new method is very promising, however, as most new statistical methods, it is often poorly understood/used. Hence, a general framework, including clear applications showing how DHGLM should be used to detect ${I * E}$ would be really helpful for the comunity, especially with the rising need to study efficiently phenotypic plasticity and it's evolution, in natural condition in a context of climate change.
