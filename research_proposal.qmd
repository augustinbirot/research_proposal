...
In order to conduct a comprehensive study of the selection acting body mass, I will use multivariate animal models to estimate genetic covariation between each age classes (@eq-mod1-ch1 & @eq-mod2-ch1).
...
Although detecting $V_{V_e}$ significantly different from 0 isn't a proof of I * E in itself , as the variation could be due to other processes, it is a necessary condition of I * E (except under very unlikely conditions @fig-opposed_unbalanced). Finding $V_{V_e} > 0$ would thus suggest that investigating I * E and looking for the enknown E is a worthwhile investigation. 
...
Based on that, I will simulate phenotypic and environmental values for populations with and without I * E, with balanced and unbalanced environmental conditions. On these simulated populations, I will fit DHGLMs models, in a Bayesian framework using R package, brms [@burknerBayesianItemResponse2021], using stan software [@carpenterStanProbabilisticProgramming2017; @stan2020rstan]. I will also investigate the potential use of TMB [@kristensenTMBAutomaticDifferentiation2016] which allows to fit DHGLMs using a frequentist approach which is much faster especially in a simulation setup but it relies on a different coding language (mixe of R and C). Then I will then apply this method on our Yellow-Bellied Marmots population to illustrate it with a real condition example.
