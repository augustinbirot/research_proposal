...
However, if the environment become too variable, simulations shows that diversified bet-hedging becomes a better option [@einum2004environmental]. Indeed, the population can mismatch with its environment [@stensethClimateChangingPhenology2002; @visserShiftsPhenologyDue2005]. Then, if phenotypic variation has decreased too much, the population looses its adaption capacity and finds itsel in an "evolutionary trap" [@schlaepferEcologicalEvolutionaryTraps2002; @robertsonEcologicalNoveltyEmergence2013].

From our data, I can extract the body mass' genetic value for each individual. I can compute the fitness associated with it (i.e., number of offspring in the individual life), therefore obtaining the fitness associated to each phenotype. I can then calculate the fitness and variance over time. If we are facing a conservative bet-hedging strategy, I expect to see a variance decrease over time [@philippi1989hedging].

Simultaneously, I will simulate two population, one having a fitness directly impacted by the environment (i.e., lots of offpsing in good years and few in poor years), called "classic", and one following a conservative bet-hedging strategy (i.e., less fitness variation, with a max fitness being inferior to "classic" population and lower fitness superior).

I will compare the predicitons of these two populations: fitness geometric means, variance to our observations. [...]
